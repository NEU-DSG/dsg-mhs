...
d3.csv('data/jqa_sentiments.csv', type).then( data => {
        ...
        bars
            ...
            .join(
                enter => enter.append('line')
                    ...
                    .attr('stroke-width', (width - margin.right) / (subset.length)) // stroke width gets larger when date range is smaller.
                    .style('opacity', 0.4)
                    .transition()
                        .duration(dur)
                        .ease(d3.easeLinear)
    ...
    circles.on('mouseover', function(event) { 
        ...
        circles
            ...
            .attr('r', (d) => {
                if (d.id === source.id) 
                {return ((width - margin.right) / (subset.length / 12))} else 
                {((width - margin.right) / (subset.length))}
            })
